...
async function withLoadedRoom(code, callback, options = {}) {
  ...
  try {
    ...
    try {
      ...
      for (const [id, dueAt] of Object.entries(room.pendingDepartures || {})) {
        if (Date.now() >= dueAt) {
...
async function leaveRoom(session, code, payload = {}) {
  ...
  return store.withRoomLock(normalized, async () => {
    ...
    else if (result.left || result.disconnecting) {
      await store.saveRoom(room, revision);
      if (result.disconnecting) await scheduler.scheduleTask(room, 'departure', result.dueAt, { playerId: session.id });
    }
    return result;
  });
}
